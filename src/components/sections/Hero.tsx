...
export function Hero() {
  ...
  useEffect(() => {
    async function loadHero() {
      try {
        const res = await fetchAPI("/hero-section", { populate: "*" })
        setData(res.data)
      } catch (err) {
        console.error(err)
      } finally {
  ...
  if (loading) {
    return (
      <section className="relative w-full h-screen flex flex-col md:flex-row overflow-hidden bg-gray-50 animate-pulse">
      </section>
    )
  }

  const title = data?.collectionTitle || "The Statuario Collection"
  const subtitle = data?.subtitle || "Premium Glazed Vitrified Tiles · 800×1600 mm"
  const bgImage = getStrapiMedia(data?.image) || "https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?w=2000&q=80"
  
  const rawHeading = data?.mainHeading || "Tiles, Bathware &\nBuilding Solutions."
  ...
  const text = data?.paragraph || "Oman's trusted destination for premium floor tiles, wall tiles, sanitaryware, faucets, adhesives, and waterproofing — curated by BLUEPRINT."
  const tags = data?.tags || ["Floor Tiles", "Wall Tiles", "Sanitaryware", "Adhesives"]
